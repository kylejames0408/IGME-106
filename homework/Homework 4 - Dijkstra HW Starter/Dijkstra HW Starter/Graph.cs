...
namespace Dijkstra_HW_Starter
{
	class Graph
	{
		...
		public void FindShortestPaths(Vertex v)
		{
			...
			for (int i = 0; i < vertices.Count; i++)
			{
				// Reset the Vertex labels
				vertices[i].Reset();
			}

			// Make a current Vertex variable
			Vertex currentVertex = v;

			// Set current Vertex labels
			currentVertex.SourceDistance = 0;
			currentVertex.Permanent = true;
			
			// While there are any non-permanent Vertices in the graph
			while (!AllPermanent())
			{
				// For each non-permanent neighbor of the current Vertex
				foreach (Vertex neighbor in NonpermanentNeighbors(currentVertex))
				{
					// Calculate cost
					int costFromCurrent = currentVertex.SourceDistance + adjMatrix[vertNameToIndex[currentVertex.Name], vertNameToIndex[neighbor.Name]];

					// If the cost from the current Vertex is less than the previous source distance
					if (costFromCurrent < neighbor.SourceDistance)
					{
						// Set the neighbor's label
						neighbor.SourceDistance = costFromCurrent;
						neighbor.PreviousVertex = currentVertex;
					}
				}

				// Find the next current - the smallest label
				Vertex nextCurrent = SmallestLabel();

				// If the next current is not null
				if (nextCurrent != null)
				{
					// Make it the current Vertex and make it permanent
		...
		/// <summary>
		/// Discovers if all Vertices are permanent.
		/// </summary>
		/// <returns>False if one Vertex is not permanent, true otherwise.</returns>
		private bool AllPermanent()
		{
			// Loop through all Vertices
			for (int i = 0; i < vertices.Count; i++)
			{
				// If the Vertex is not permanent
				if (!vertices[i].Permanent)
				{
					// Return false; they are not all permanent
					return false;
				}
			}

			// If it completes the loop without returning false, they are all permanent
			return true;
		}

		/// <summary>
		/// Gets the non-permanent neighbors to a Vertex.
		/// </summary>
		/// <param name="vertex">The Vertex to check for neighbors.</param>
		/// <returns>A list of non-permanent neighbors in relation to the Vertex.</returns>
		private List<Vertex> NonpermanentNeighbors(Vertex vertex)
		{
			// Temporary field
			...
			{
				// If the Vertex is adjacent to the passed Vertex
				// And if the Vertex is not permanent
				if (adjMatrix[vertNameToIndex[vertex.Name], i] > 0
					&& !vertices[i].Permanent)
				{
					// Add the Vertex to the list of eligible neighbors
					nonpermanentNeighbors.Add(vertices[i]);
				}
			}

			// Return the list of eligible neighbors
			return nonpermanentNeighbors;
		}

		/// <summary>
		/// Finds the nonpermanent Vertex with the smallest distance label.
		/// </summary>
		/// <returns>The nonpermanent Vertex with the smallest distance label or null if everything is permanent.</returns>
		private Vertex SmallestLabel()
		{
			// Temporary fields
			...
			for (int i = 0; i < vertices.Count; i++)
			{
				// If the Vertex has the smallest distance
				// And if the Vertex is non-permanent
				if (vertices[i].SourceDistance < smallestDistance
					&& !vertices[i].Permanent)
				{
					// Set the smallest distance and Vertex
					smallestDistance = vertices[i].SourceDistance;
					vertex = vertices[i];
				}
			}

			// Return the Vertex with the smallest distance or null
			return vertex;
		}
